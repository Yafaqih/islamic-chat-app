...
export default function IslamicChatApp() {
  ...
  // Thèmes islamiques avec mots-clés et suggestions
  const islamicThemes = {
    companions: {
  ...
  // Charger et mettre à jour les préférences utilisateur
  const updateUserPreferences = useCallback((question) => {
  ...
  // Générer des suggestions intelligentes basées sur les préférences
  const generateSmartSuggestions = useCallback(() => {
    ...
    // Si pas assez d'historique, utiliser les suggestions par défaut
    ...
    // Trier les thèmes par score
    const sortedThemes = Object.entries(prefs)
      .sort(([, a], [, b]) => b - a)
      .slice(0, 4)
      .map(([theme]) => theme);
    
    // Générer des suggestions variées
    ...
    sortedThemes.forEach(theme => {
      if (islamicThemes[theme] && newSuggestions.length < 4) {
        ...
        // Prendre une suggestion aléatoire de ce thème
    ...
    // Compléter avec des suggestions par défaut si nécessaire
  ...
  }, [user?.email, language, defaultSuggestions]);

  // Générer les suggestions au chargement et quand l'utilisateur change
  useEffect(() => {
    generateSmartSuggestions();
  }, [generateSmartSuggestions]);

  // ===== FIN SYSTÈME SUGGESTIONS INTELLIGENTES =====
  ...
  const handleSend = async (directMessage = null) => {
    ...
    // Mettre à jour les préférences utilisateur (suggestions intelligentes)
